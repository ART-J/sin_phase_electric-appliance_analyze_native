...
__interrupt void adc1_isr(void)
{
    ...
    if (2048 <= adc1_index) {
        ...
        if (adc1_buffer_read == 1) {                //判ADC读取标志若为1，当前为读取状态执行操作
            adc1_buffer_read = 0;
...
__interrupt void adc2_isr(void)
{
    ...
    if (2048 <= adc2_index) {
        ...
        if (adc2_buffer_read == 1) {     //adc2_buffer_read初始值为1，若ADC采集完值，且adc2_buffer_read为1则复制adc2_results值到adc2_buffer_sample 数组
            adc2_buffer_read = 0;
